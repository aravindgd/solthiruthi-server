class DictionariesController < ApplicationController
  ...
  def index
    ...
    if @searchText.blank?
      ...
      @dictionaries = Dictionary.where((table[:word].matches(search_pattern)).or(table[:meaning].matches(search_pattern)))
    end
    @dictionaries = @dictionaries.page(params[:page]).per(5)
  end
